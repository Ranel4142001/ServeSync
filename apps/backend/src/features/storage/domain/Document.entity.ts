...
// A Document represents a file that was uploaded
// and attached to a ticket — e.g. a screenshot,
// a contract PDF, or an invoice image
interface DocumentProps {
  fileName:  string; // original name e.g. "invoice.pdf"
  s3Key:     string; // the unique path inside S3 bucket
                     // e.g. "uploads/org123/ticket456/invoice.pdf"
  mimeType:  string; // file type e.g. "application/pdf", "image/png"
  sizeBytes: number; // file size in bytes e.g. 204800 (200KB)
  ticketId:  string; // which ticket this file is attached to
  createdAt: Date;
}
...
export class Document extends Entity<string> {
  ...
  // ── Factory method ───────────────────────────────────────
  // Validates the document before it is created
  static create(props: DocumentProps, id?: string): Document {
    ...
    // S3 key must exist — it is generated before calling create()
    if (!props.s3Key || props.s3Key.trim().length === 0) {
      throw new Error('S3 key is required');
    }

    // File must have a valid mime type
